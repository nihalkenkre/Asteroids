...
common_graphics::common_graphics (const HINSTANCE& h_instance, const HWND& h_wnd)
{
    ...
    graphics_device = std::make_unique<vk_graphics_device> (
        physical_device->physical_device,
        queue_infos->queue_create_infos
        );
    device_queues = std::make_unique<vk_device_queues> (
        physical_device->physical_device,
    ...
    transfer_command_pool = std::make_unique<vk_command_pool> (
        graphics_device->graphics_device,
        queue_family_indices->transfer_queue_family_index,
        VK_COMMAND_POOL_CREATE_TRANSIENT_BIT
        );
    common_sampler = std::make_unique<vk_sampler> (graphics_device->graphics_device);
}